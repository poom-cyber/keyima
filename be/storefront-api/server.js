...
app.get("/api/settings", (_q, res) => {
  ...
  res.json({ shopName: s.shopName, promo: s.promo, shippingFlat: s.shippingFlat, freeShipMin: s.freeShipMin, heroProductId: s.heroProductId, heroProductIds: s.heroProductIds, homeSections: s.homeSections, seriesOrder: s.seriesOrder });
});
...
/* ลูกค้าแนบสลิปทีหลัง (guest) — เฉพาะออเดอร์ที่ยังไม่มีสลิป และยังรอชำระ */
app.post("/api/orders/track/slip", (req, res) => {
  const no = String(req.body.no || "").trim();
  const email = String(req.body.email || "").trim().toLowerCase();
  const slip = typeof req.body.slip === "string" ? req.body.slip : "";
